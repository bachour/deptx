...
class Unit(models.Model):
    ...
    mail_revoking_document = models.TextField(default="Document {{data}} was not processed in time. The document has been revoked.", help_text="when a document is being revoked")
...
#         (CATEGORY_TASK, "request task"),
    ...
    TYPE_NORMAL = 0
    TYPE_INITIAL = 1
    TYPE_TUTORIAL_REQUEST = 2
    TYPE_TUTORIAL_SUBMIT = 3
    
    CHOICES_TYPE = (
        (TYPE_NORMAL, "normal"),
        (TYPE_INITIAL, "initial"),
        (TYPE_TUTORIAL_REQUEST, "tutorial request"),
        (TYPE_TUTORIAL_SUBMIT, "tutorial submit"),
    )
    ...
    def get_category_acr(self):
        if self.category == self.CATEGORY_FORM:
            return "AD"
        elif self.category == self.CATEGORY_DOCUMENT:
            return "DC"
        elif self.category == self.CATEGORY_SUBMISSION:
            return "SU"
        else:
            return "XX"
        
    def clean(self, *args, **kwargs):
        if self.type == self.TYPE_TUTORIAL_REQUEST:
            try:
                requisition = Requisition.objects.get(type=Requisition.TYPE_TUTORIAL_REQUEST)
                raise ValidationError('You can only have one form set as the tutorial form! Change form %s first!' % requisition.serial)
            except Requisition.DoesNotExist:
                pass
        elif self.type == self.TYPE_TUTORIAL_SUBMIT:
            try:
                requisition = Requisition.objects.get(type=Requisition.TYPE_TUTORIAL_SUBMIT)
                raise ValidationError('You can only have one form set as the tutorial form! Change form %s first!' % requisition.serial)
            except Requisition.DoesNotExist:
                pass
...
class Mission(models.Model):
    ...
    isLastMission = models.BooleanField(default=False)
...
class CronDocument(AbstractDocument):
    case = models.ForeignKey(Case, related_name="crondocument", blank=True, null=True)
