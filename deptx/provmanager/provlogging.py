...
from provmanager.views import API
...
def addBundle(cron, bundle, bundle_id):
    ...
    bundle_id = bundle_id + "/" + str(datetime.datetime.now())
    print bundle_id
...
def create_bundle_cron_login(user_id, session_key):
    ...
    now = datetime.datetime.now()
    a = b.activity('log:%d/%s' % (user_id, 'login'), now, now, other_attributes=[('prov:type', 'act:CronAccountLogin')])
...
def create_bundle_cron_logout(user_id, session_key):
    ...
    ag = b.agent('cronuser:%d/%s' % (user_id, session_key))
    now = datetime.datetime.now()
    a = b.activity('ns:logout', now, now, other_attributes=[('prov:type', 'act:CronAccountLogout')])
...
def create_bundle_mop_login(user_id, session_key):
    ...
    now = datetime.datetime.now()
    a = b.activity('log:%d/%s' % (user_id, 'login'), now, now, other_attributes=[('prov:type', 'act:MopAccountLogin')])
    b.wasAssociatedWith(a, u)
    b.wasGeneratedBy(ag, a)
    b.specializationOf(ag, u)
    return bundle_id, b
...
def create_bundle_mop_logout(user_id, session_key):
    ...
    ag = b.agent('mopuser:%d/%s' % (user_id, session_key))
    now = datetime.datetime.now()
    a = b.activity('ns:logout', now, now, other_attributes=[('prov:type', 'act:MopAccountLogout')])
    b.wasInvalidatedBy(ag, a)  # This user+session no longer exists after this
    return bundle_id, b
